...
/** @fn fsal_status_t
 *	fsal_internal_handle2fd(int dir_fd, struct gpfs_file_handle *gpfs_fh, int *fd,
 *				int oflags, bool reopen)
 *  @brief Open a file by handle within an export.
 *
 *  @param dir_fd Descriptor
			...
			int oflags, bool reopen)
{
	...
	return fsal_internal_handle2fd_at(dir_fd, gpfs_fh, fd, oflags, reopen);
}

/** @fn fsal_status_t fsal_internal_close(int fd, void *owner, int cflags)
 *  @brief Close by fd
 *
...
/** @fn fsal_status_t
 *	fsal_internal_handle2fd_at(int dir_fd, struct gpfs_file_handle *gpfs_fh, int *fd,
 *				   int oflags, bool reopen)
...
fsal_status_t
fsal_internal_handle2fd_at(int dir_fd, struct gpfs_file_handle *gpfs_fh,
			    ...
			    struct gpfs_file_handle *gpfs_fh)
{
	...
	LogFullDebug(COMPONENT_FSAL, "Lookup handle at for %d %s", fd,
		     fs_name);
...
/** @fn fsal_status_t
 *	fsal_internal_get_fh(int dir_fd, struct gpfs_file_handle *gpfs_fh,
 *			     const char *fs_name,
...
/** @fn fsal_status_t fsal_internal_fd2handle(int fd,
 *					      struct gpfs_file_handle *gpfs_fh)
...
/** @fn fsal_status_t
 *	fsal_internal_link_fh(int dir_fd, struct gpfs_file_handle *gpfs_fh_tgt,
 *			      struct gpfs_file_handle *gpfs_fh,
...
/** @fn fsal_status_t
 *	fsal_internal_unlink(int dir_fd, struct gpfs_file_handle *gpfs_fh,
 *			     const char *stat_name, struct stat *buf)
...
/** @fn fsal_status_t
 *	fsal_internal_create(struct fsal_obj_handle *dir_hdl,
 *			     const char *stat_name, mode_t mode, dev_t dev,
 *			     struct gpfs_file_handle *gpfs_fh, struct stat *buf)
 ...
 *  @param stat file attributes of new file
 *
 *  @return status of operation
 */
fsal_status_t
fsal_internal_create(struct fsal_obj_handle *dir_hdl, const char *stat_name,
		     mode_t mode, dev_t dev, struct gpfs_file_handle *gpfs_fh,
		     struct stat *buf)
{
...
/** @fn fsal_status_t
 *	fsal_internal_rename_fh(int dir_fd, struct gpfs_file_handle *gpfs_fh_old,
...
/** @fn fsal_status_t
 *	fsal_readlink_by_handle(int dir_fd, struct gpfs_file_handle *gpfs_fh,
...
/** @fn int fsal_internal_version(void)
...
/** @fn fsal_status_t
 *	fsal_get_xstat_by_handle(int dir_fd, struct gpfs_file_handle *gpfs_fh,
 *				 gpfsfsal_xstat_t *buffxstat,
 *				 uint32_t *exp_tattr, bool expire, bool use_acl)
 *  @brief Get NFS4 ACL as well as stat.
 *
 *  @param dir_fd Open file descriptor of parent directory
 *  @param gpfs_fh file handle of file
 *  @param buffxstat Buffer
 *  @param exp_tattr Expire time attributes
...
fsal_status_t
fsal_get_xstat_by_handle(int dir_fd, struct gpfs_file_handle *gpfs_fh,
			 gpfsfsal_xstat_t *buffxstat, uint32_t *exp_tattr,
...
/** @fn fsal_status_t
 *	fsal_set_xstat_by_handle(int dir_fd,
 ...
 */
fsal_status_t
fsal_set_xstat_by_handle(int dir_fd, const struct req_op_context *op_ctx,
...
/** @fn fsal_status_t
 *	fsal_trucate_by_handle(int dir_fd, const struct req_op_context *op_ctx,
		       ...
		       struct gpfs_file_handle *gpfs_fh, u_int64_t size)
{
	...
	return fsal_set_xstat_by_handle(dir_fd, op_ctx, gpfs_fh, XATTR_STAT,
					XATTR_SIZE, &buffxstat);
}

/** @fn bool fsal_error_is_event(fsal_status_t status)
...
/** @fn bool fsal_error_is_info(fsal_status_t status)
