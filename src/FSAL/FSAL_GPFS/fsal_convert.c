...
/** @fn void posix2fsal_attributes(const struct stat *buf,
...
/** @fn static int gpfs_acl_2_fsal_acl(struct attrlist *obj_attr,
 *				       gpfs_acl_t *gpfs_acl)
 *  @param obj_attr Object attributes
 *  @param gpfs_acl GPFS access control list
 *  @return ERR_FSAL_NO_ERROR on success, ERR_FSAL_FAULT otherwise
 */
				 ...
				 struct attrlist *fsal_attr, bool use_acl)
{
	...
	if (FSAL_TEST_MASK(fsal_attr->mask, ATTR_FILEID)) {
		fsal_attr->fileid = (uint64_t) (buf->st_ino);
		LogFullDebug(COMPONENT_FSAL, "fileid = %lu", fsal_attr->fileid);
	}

	if (FSAL_TEST_MASK(fsal_attr->mask, ATTR_MODE)) {
		fsal_attr->mode = unix2fsal_mode(buf->st_mode);
		LogFullDebug(COMPONENT_FSAL, "mode = %"PRIu32, fsal_attr->mode);
	}
	if (FSAL_TEST_MASK(fsal_attr->mask, ATTR_NUMLINKS)) {
		fsal_attr->numlinks = buf->st_nlink;
	...
	if (FSAL_TEST_MASK(fsal_attr->mask, ATTR_OWNER)) {
		fsal_attr->owner = buf->st_uid;
		LogFullDebug(COMPONENT_FSAL, "owner = %lu", fsal_attr->owner);
	}
	if (FSAL_TEST_MASK(fsal_attr->mask, ATTR_GROUP)) {
		fsal_attr->group = buf->st_gid;
		LogFullDebug(COMPONENT_FSAL, "group = %lu", fsal_attr->group);
	}
	if (FSAL_TEST_MASK(fsal_attr->mask, ATTR_ATIME)) {
		fsal_attr->atime = posix2fsal_time(buf->st_atime,
						   buf->st_atim.tv_nsec);
	...
	if (FSAL_TEST_MASK(fsal_attr->mask, ATTR_MTIME)) {
		fsal_attr->mtime = posix2fsal_time(buf->st_mtime,
						   buf->st_mtim.tv_nsec);
		    ...
		    gpfsfsal_xstat_t *gpfs_buf)
{
	...
	for (ace = fsal_acl->aces; ace < (fsal_acl->aces + fsal_acl->naces);
	     ace++, i++) {
