...
/** @fn static void fs_layouttypes(struct fsal_export *export_hdl,
 ...
 *
 *  @param[in]  export_hdl Public export handle
			   ...
			   const layouttype4 **types)
{
	...
	struct gpfs_fsal_export *myself =
		container_of(export_hdl, struct gpfs_fsal_export, export);
	struct gpfs_filesystem *gpfs_fs = myself->root_fs->private;
	struct open_arg arg = {0};
	int errsv = 0;
	int rc = 0;
...
/** @fn static uint32_t fs_layout_blocksize(struct fsal_export *export_pub)
...
/** @fn static uint32_t fs_maximum_segments(struct fsal_export *export_pub)
...
/** @fn static size_t fs_loc_body_size(struct fsal_export *export_pub)
...
/** @fn size_t fs_da_addr_size(struct fsal_module *fsal_hdl)
...
/** @fn nfsstat4
 *	getdeviceinfo(struct fsal_module *fsal_hdl, XDR *da_addr_body,
...
nfsstat4
getdeviceinfo(struct fsal_module *fsal_hdl, XDR *da_addr_body,
	      const layouttype4 type, const struct pnfs_deviceid *deviceid)
{
	struct deviceinfo_arg darg = {0};
	/** The position before any bytes are sent to the stream */
	size_t da_beginning = xdr_getpos(da_addr_body);
	size_t ds_buffer = da_addr_body->x_handy;
	size_t da_length = 0;
	int rc = 0;
	...
	LogDebug(COMPONENT_PNFS,
		"getdeviceinfo p %p end %p da_beginning %lu ds_buffer %lu seq %d fd %d fsid 0x%lx\n",
 ...
 *
 *  We do not support listing devices and just set EOF without doing anything.
 *
 ...
 */
static nfsstat4
getdevicelist(struct fsal_export *export_pub, layouttype4 type, void *opaque,
...
/** fn void export_ops_pnfs(struct export_ops *ops)
 *
 */
...
/** @fn static inline void fh_log_debug(struct gpfs_file_handle *gpfs_fh)
 */
static inline void fh_log_debug(struct gpfs_file_handle *gpfs_fh)
{
	unsigned *fh = (int *)&(gpfs_fh->f_handle);
...
/** @fn static nfsstat4
 *	layoutget(struct fsal_obj_handle *dir_hdl, struct req_op_context *op_ctx,
 ...
 *  ignore it otherwise.
 *
 *  @param[in]     dir_hdl  Public object handle
 *  @param[in]     op_ctx   Request context
	  ...
	  struct fsal_layoutget_res *res)
{
	...
	struct gpfs_file_handle gpfs_fh = {0};
	struct layoutget_arg lay_arg = {0};
	struct layoutreturn_arg lay_ret_arg = {0};
	struct pnfs_filelayout_layout file_layout = {0};
	uint32_t stripe_width = 0;  /** Width of each stripe on the file */
	nfl_util4 util = 0;
	...
	memcpy(&gpfs_fh, myself->handle, sizeof(struct gpfs_file_handle));

	lay_arg.fd = myself->u.file.fd;
	lay_arg.args.lg_minlength = arg->minlength;
	lay_arg.args.lg_sbid = arg->export_id;
	lay_arg.args.lg_fh = &gpfs_fh;
	lay_arg.args.lg_iomode = res->segment.io_mode;
	lay_arg.handle = &gpfs_fh;
	lay_arg.file_layout = &file_layout;
	lay_arg.xdr = NULL;

	fh_log_debug(&gpfs_fh);

	rc = gpfs_ganesha(OPENHANDLE_LAYOUT_GET, &lay_arg);
	errsv = errno;
	if (rc != 0) {
	...
	lay_ret_arg.mountdirfd = myself->u.file.fd;
	lay_ret_arg.handle = &gpfs_fh;
	lay_ret_arg.args.lr_return_type = arg->type;
	lay_ret_arg.args.lr_reclaim = false;
	lay_ret_arg.args.lr_seg.clientid = 0;
	lay_ret_arg.args.lr_seg.layout_type = arg->type;
	lay_ret_arg.args.lr_seg.iomode = res->segment.io_mode;
	lay_ret_arg.args.lr_seg.offset = 0;
	lay_ret_arg.args.lr_seg.length = NFS4_UINT64_MAX;

	rc = gpfs_ganesha(OPENHANDLE_LAYOUT_RETURN, &lay_ret_arg);
	errsv = errno;
	LogDebug(COMPONENT_PNFS, "GPFSFSAL_layoutreturn rc %d", rc);
	if (rc != 0 && errsv == EUNATCH)
		LogFatal(COMPONENT_PNFS, "GPFS Returned EUNATCH");
...
/** @fn static nfsstat4
 *	layoutreturn(struct fsal_obj_handle *dir_hdl, struct req_op_context *op_ctx,
 *		     XDR *lrf_body, const struct fsal_layoutreturn_arg *arg)
	     ...
	     XDR *lrf_body, const struct fsal_layoutreturn_arg *arg)
{
	...
	struct layoutreturn_arg larg = {0};
	int errsv = 0;
	int rc = 0;
	...
	larg.args.lr_reclaim = (arg->circumstance == circumstance_reclaim);
...
/** @fn static nfsstat4
 *	layoutcommit(struct fsal_obj_handle *dir_hdl, struct req_op_context *op_ctx,
 *		     XDR *lou_body, const struct fsal_layoutcommit_arg *arg,
	     ...
	     struct fsal_layoutcommit_res *res)
{
	struct gpfs_fsal_obj_handle *myself =
		container_of(dir_hdl, struct gpfs_fsal_obj_handle, obj_handle);
	struct gpfs_file_handle *gpfs_fh = myself->handle;
	struct layoutcommit_arg targ = {0};
	int errsv = 0;
	int rc = 0;
	...
	targ.reclaim = arg->reclaim;		/* True if reclaim commit */
	targ.new_offset = arg->new_offset;	/* True if the client has
						   suggested a new offset */
	if (arg->new_offset)
...
/** @fn void handle_ops_pnfs(struct fsal_obj_ops *ops)
 */
